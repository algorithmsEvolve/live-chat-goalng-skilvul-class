...
func main() {
	...
	r.POST("/login", login)
	r.POST("/change_profile", change_profile)
	r.POST("/change_password", change_password)
	r.Run()
}
...
func change_password(c *gin.Context) {
	...
	h.Write([]byte(new_password))
	new_password = fmt.Sprintf("%x", h.Sum(nil))

	_, err_up := db.Exec(query_change_password, new_password, username)
	if err_up != nil {
		c.JSON(400, StandardAPIResponse{
			Err: err_up.Error(),
		})
